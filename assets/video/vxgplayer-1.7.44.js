window.vxgplayer = function(id){
	window.vxgplayer.version="1.7.44"; //version is updated by 'npm run build'
	...
	if(!window.vxgplayer.players[id]){
		...
		if(!vxgplayer.browserSupportsPluginPnacl()){
			...
			+ '<div class="vxgplayer-unsupport">'
			+ '	<div class="vxgplayer-unsupport-content">'
			+ '	<a href="http://rtpstream.com/player_start/">Click here for install plugin</a>'
		...
		// magic run app
		window.location.href = "http://rtpstream.com/player_start/";
		window.vxgplayer.players[id] = new function(id){
			var self = this;
			self.id = id;
			...
			var url = "";
			self.playerWidth=640;
			self.playerHeight=480;

			nmf_path = self.player.getAttribute('nmf-path') || nmf_path;
			nmf_src = self.player.getAttribute('nmf-src') || nmf_src;
			url = self.player.getAttribute('url');
			...
			self.module.avsync = false;
			self.module.volume = 0.7;
			self.module.vxgReadyState = 0;
			self.module.autohide = 2000;
			...
			self.module.autoreconnect = 0;
			if(self.player.hasAttribute('auto-reconnect')){
			...
			self.module.mute = self.module.volume == 0;
			if(self.module.mute){
